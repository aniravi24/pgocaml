  ...
  */
  ...
  let string_of_timestamptz = ((cal, tz)) =>
    ...
    ++ (
      ...
      | Time_Zone.UTC => "+00"
      | Time_Zone.Local =>
        let gap = Time_Zone.gap(Time_Zone.UTC, Time_Zone.Local);
        if (gap >= 0) {
          sprintf("+%02d", gap);
        } else {
          sprintf("-%02d", - gap);
  ...
  let string_of_interval = p => {
    let (y, m, d, s) = Calendar.Period.ymds(p);
  ...
  let timestamptz_of_string = str => {
    ...
    let n = String.length(str);
    let (cal, tz) =
      if (n >= 3 && (str.[n - 3] == '+' || str.[n - 3] == '-')) {
        (String.sub(str, 0, n - 3), Some(String.sub(str, n - 3, 3)));
      } else {
        (str, None);
      };
    let cal = timestamp_of_string(cal);
    let tz =
      switch (tz) {
      ...
      | Some(tz) =>
        ...
        let mag = int_of_string(String.sub(tz, 1, 2));
        Time_Zone.UTC_Plus(sgn * mag);
      };
    (cal, tz);
  };
