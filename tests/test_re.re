...
let inet_of_string = {
  let printer = ((inet, addr)) =>
    sprintf("%s/%d", Unix.string_of_inet_addr(inet), addr);
  [
  ...
  |> List.map(((s, (str, mask))) => {
       let ip = Unix.inet_addr_of_string(str);
       s
       >:: (
...
let point_of_string = {
  let printer = ((f1, f2)) => sprintf("(%f, %f)", f1, f2);
  let tests =
    [
      ("(0,0)", (0.0, 0.0)),
      ("(+1,-2)", (1.0, (-2.0))),
    ...
    |> List.map(((s, pt)) =>
         s >:: (() => assert_equal(~printer, pt, PGOCaml.point_of_string(s)))
       );
  [
    "nan" >:: (() => ignore(PGOCaml.point_of_string("(nan ,  NaN )"))),
    ...tests,
  ];
};

let interval_of_string = {
  let printer = PGOCaml.string_of_interval;
  module Period = CalendarLib.Calendar.Period;
  let add' = List.fold_left(Period.add, Period.empty);
  [
    ...
    (
      "12 year 00:12:03",
      Period.(add'([year(12), minute(12), second(3)])),
    ),
    ("77 day 12:12", Period.(add'([day(77), hour(12), minute(12)]))),
  ]
  |> List.map(((s, p)) =>
       s >:: (() => assert_equal(~printer, p, PGOCaml.interval_of_string(s)))
     );
};
