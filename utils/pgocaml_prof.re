...
let () = {
  ...
  let (get_chan, close_chan) = {
    ...
    let get_chan = conn =>
      ...
      | _ =>
        close_chan();
        let open_flags = [Open_wronly, Open_append, Open_creat];
        let filename = tmpdir /\/ conn;
        let chan = open_out_gen(open_flags, 420, filename);
        last_chan := Some(chan);
        last_conn := Some(conn);
        chan;
      };
...
type query_data = {
  ...
  qprogs: list(string), /* Programs which used this. */
...
type connection_data = {
  ...
  progs: list(string), /* Programs which used this. */
...
let () =
  List.iter(
    uuid => {
      let rows = Csv.load(tmpdir /\/ uuid);
      nr_rows' := nr_rows'^ + List.length(rows);

      let ignore = msg => raise(Ignore(msg));
      try (
        {
          assert(rows != []);
          ...
          let (params, prog) =
            ...
            | _ =>
              ignore(
                sprintf(
                  "connection %s did not start with a 'connect' operation",
                  uuid,
                ),
              )
            };

          set_connection(params, data =>
            {
              ...data,
          ...
          let qnames = Hashtbl.create(13);

          List.iter(
            fun
            | ["1", "connect", time, status, ...details] => {
                let time = int_of_string(time);
                let failures =
                  if (status == "ok") {
                    0;
                  } else {
            ...
            | ["1", "execute", time, status, ...details] => {
                ...
                let query =
                  ...
                  | Not_found =>
                    ignore(
                      sprintf("execute on unprepared query name '%s'", name),
                    )
                  };
                set_query(query, data =>
                  {
            ...
            | _ => ignore("invalid row"),
            rows,
          );
        }
      ) {
      | Ignore(msg) => eprintf("warning: %s\n", msg)
      };
    },
    files,
  );
...
let () = {
  List.iter(filename => Unix.unlink(tmpdir /\/ filename), files);
  Unix.rmdir(tmpdir);
};
...
let queries =
  Hashtbl.fold((query, data, xs) => [(query, data), ...xs], queries, []);
let connections =
  Hashtbl.fold(
    (params, data, xs) => [(params, data), ...xs],
    connections,
    [],
  );
